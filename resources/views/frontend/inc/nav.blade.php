...
@endif
<!-- Top Bar -->
<div class="top-navbar bg-primary text-white border-bottom border-soft-secondary z-1035">
    <div class="container">
        <div class="row">
                    ...
                    @auth
                        @if(isAdmin())
                            <li class="list-inline-item mr-3 border-right border-left-0 pr-3 pl-0">
                                <a href="{{ route('admin.dashboard') }}" class="text-white d-inline-block py-2">{{ translate('My Panel')}}</a>
                            </li>
                        @else
                            <li class="list-inline-item mr-3 border-right border-left-0 pr-3 pl-0">
                                <a href="https://tajjar.pk/support_ticket" class="text-white d-inline-block py-2"> {{ translate('Support Ticket') }}</a>
                            </li>
                            <li class="list-inline-item mr-3 border-right border-left-0 pr-3 pl-0">
                                <a href="{{ route('orders.track') }}" class="text-white d-inline-block py-2"> {{ translate('Track Order') }}</a>
                            </li>
                            <li class="list-inline-item mr-3 border-right border-left-0 pr-3 pl-0">
                                <a class="dropdown-toggle text-white no-arrow d-inline-block py-2" data-toggle="dropdown" href="javascript:void(0);" role="button" aria-haspopup="false" aria-expanded="false">
                            ...
                            <li class="list-inline-item mr-3 border-right border-left-0 pr-3 pl-0">
                                <a href="{{ route('dashboard') }}" class="text-white d-inline-block py-2">{{ translate('My Panel')}}</a>
                            </li>
                        @endif
                        <li class="list-inline-item">
                            <a href="{{ route('logout') }}" class="text-white d-inline-block py-2">{{ translate('Logout')}}</a>
                        </li>
                    @else
                        <li class="list-inline-item mr-3 border-right border-left-0 pr-3 pl-0">
                            <a href="{{ route('shops.create') }}" class="text-white d-inline-block py-2"> {{ translate('Be a Seller') }}</a>
                        </li>
                        <li class="list-inline-item mr-3 border-right border-left-0 pr-3 pl-0">
                            <a href="{{ route('orders.track') }}" class="text-white d-inline-block py-2"> {{ translate('Track Order') }}</a>
                        </li>
                        <li class="list-inline-item mr-3 border-right border-left-0 pr-3 pl-0">
                            <a href="{{ route('user.login') }}" class="text-white d-inline-block py-2">{{ translate('Login')}}</a>
                        </li>
                        <li class="list-inline-item">
                            <a href="{{ route('user.registration') }}" class="text-white d-inline-block py-2">{{ translate('Registration')}}</a>
                        </li>
                    @endauth
                </ul>
            </div>
...
<header class="@if(get_setting('header_stikcy') == 'on') sticky-top @endif z-1020 bg-white border-bottom shadow-sm">
            ...
            <div class="d-flex align-items-center">
                <div class="col-auto col-xl-3 pl-0 pr-3 d-flex align-items-center">
                    <a class="d-block py-5px mr-3 ml-0" href="{{ route('home') }}">
                        ...
                        @endphp
                        @if($header_logo != null)
                            <img src="{{ uploaded_asset($header_logo) }}" alt="{{ env('APP_NAME') }}" class="mw-100 h-30px h-md-50px" height="40">
                        @else
                            ...
                            <div class="d-flex position-relative align-items-center">
                                ...
                                <div class="input-group">
                                    <input type="text" class="border-0 border-lg form-control" id="search" name="keyword" @isset($query)
                                        value="{{ $query }}"
                                    @endisset placeholder="{{translate('What are you looking for...')}}" autocomplete="off">
    ...
    @endif
    <div class="bg-gray border-top border-gray-200 py-1">
        <div class="container">
                <ul class="list-inline mb-0 pl-0 mobile-hor-swipe">
                    @if(Route::currentRouteName() != 'home')
                        <li class="list-inline-item mr-0">
                            <div id="category-menu-icon" class="dropdown-toggle navbar-light bg-light h-40px pl-2 rounded border c-pointer">
                                <a href="#" class="opacity-60 fs-14 d-inline-block fw-600 hov-opacity-100 text-reset">
